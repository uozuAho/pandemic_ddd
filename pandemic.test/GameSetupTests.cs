...
public class GameSetup
{
    ...
    public void Initial_game_state_is_correct()
    {
        ...
        Assert.That(
            game.PlayerDrawPile.Cards.Count(c => c is EpidemicCard) == numberOfEpidemicCards
        );
        ...
        Assert.That(game.Cubes.Total(), Is.EqualTo(96 - (3 * 3) - (3 * 2) - (3 * 1)));
    ...
    private static void AssertEpidemicCardsAreDistributed(
        Deck<PlayerCard> drawPile,
        Difficulty difficulty
    )
    {
        foreach (
            var chunk in drawPile.Cards.SplitEvenlyInto(
                PandemicGame.NumberOfEpidemicCards(difficulty)
            )
        )
        {
