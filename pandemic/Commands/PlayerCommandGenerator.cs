...
public static class PlayerCommandGenerator
{
    ...
    private static IEnumerable<IPlayerCommand> AllCommands(PandemicGame game, bool beSensible)
    {
        ...
        foreach (var c in DiscardCommands(game))
        {
            numCmds++;
            yield return c;
        }
        if (beSensible)
        {
            foreach (var c in SensibleSpecialEventCommands(game))
            {
                numCmds++;
                numEventCmds++;
                yield return c;
        ...
        {
            foreach (var c in SpecialEventCommands(game))
            {
                numCmds++;
                numEventCmds++;
                yield return c;
        ...
        {
            foreach (var c in DriveFerryCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in BuildResearchStationCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in CureCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in DirectFlightCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in CharterFlightCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in ShuttleFlightCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in TreatDiseaseCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in ShareKnowledgeGiveCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in ShareKnowledgeTakeCommands(game))
            {
                numCmds++;
                yield return c;
            }
            if (!beSensible)
            {
                foreach (var c in PassCommands(game))
                {
                    numCmds++;
                    yield return c;
                }
            }
            foreach (var c in DispatcherCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in OperationsExpertCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in ResearcherCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in ScientistCommands(game))
            {
                numCmds++;
                yield return c;
            }
            foreach (var c in ContingencyPlannerTakeCommands(game))
            {
                numCmds++;
                yield return c;
            }
    ...
    {
        if (
            game.CurrentPlayer.Role != Role.ContingencyPlanner
            || game.CurrentPlayer.ActionsRemaining == 0
            || game.PhaseOfTurn != TurnPhase.DoActions
        )
        {
            yield break;
        }

        if (game.ContingencyPlannerStoredCard != null)
        {
            yield break;
        }
        ...
        foreach (
            var card in game
                .PlayerDiscardPile.Cards.Where(c => c is ISpecialEventCard)
                .Cast<ISpecialEventCard>()
        )
        {
    ...
    {
        if (
            game.CurrentPlayer.Role != Role.Scientist
            || game.CurrentPlayer.ActionsRemaining == 0
            || game.PhaseOfTurn != TurnPhase.DoActions
        )
        {
    ...
    {
        if (game.CurrentPlayer.ActionsRemaining == 0 || game.PhaseOfTurn != TurnPhase.DoActions)
        {
            yield break;
        }

        if (!game.Players.Any(p => p.Role == Role.Researcher))
        {
            yield break;
        }
        ...
        foreach (var card in researcher.Hand.CityCards())
        {
            ...
            {
                foreach (
                    var otherPlayer in game.Players.Where(p =>
                        p.Role != Role.Researcher && p.Location == researcher.Location
                    )
                )
                {
                    yield return new ResearcherShareKnowledgeGiveCommand(
                        otherPlayer.Role,
                        card.City.Name
                    );
                }
            ...
            {
                yield return new ShareKnowledgeTakeFromResearcherCommand(
    ...
    {
        if (
            game.CurrentPlayer.Role != Role.OperationsExpert
            || game.CurrentPlayer.ActionsRemaining == 0
            || game.PhaseOfTurn != TurnPhase.DoActions
        )
        {
    ...
    {
        if (
            game.CurrentPlayer.Role != Role.Dispatcher
            || game.CurrentPlayer.ActionsRemaining == 0
            || game.PhaseOfTurn != TurnPhase.DoActions
        )
        {
        ...
        foreach (var x in DispatcherMovePawnToOtherPawns(game))
        {
            yield return x;
        }

        foreach (var x in DispatcherDriveFerryCommands(game))
        {
            yield return x;
        }

        foreach (var x in DispatcherDirectFlightCommands(game))
        {
            yield return x;
        }

        foreach (var x in DispatcherCharterFlightCommands(game))
        {
            yield return x;
        }

        foreach (var x in DispatcherShuttleFlightCommands(game))
        {
            yield return x;
        }
    }
    ...
    private static IEnumerable<IPlayerCommand> DispatcherShuttleFlightCommands(PandemicGame game)
    {
        ...
        {
            if (otherPlayer.Role == Role.Dispatcher)
            {
                continue;
            }
            ...
            {
                foreach (
                    var city in game.Cities.Where(c =>
                        c.Name != otherPlayer.Location && c.HasResearchStation
                    )
                )
                {
            ...
            for (var j = 0; j < dispatcherHand.Count; j++)
            {
                ...
                if (card is PlayerCityCard cityCard && otherPlayer.Location == cityCard.City.Name)
                {
                    for (var k = 0; k < game.Cities.Length; k++)
                    {
                        ...
                        {
                            yield return new DispatcherCharterFlyPawnCommand(
                                otherPlayer.Role,
                                city.Name
                            );
                        }
    ...
    private static IEnumerable<IPlayerCommand> DispatcherDirectFlightCommands(PandemicGame game)
    {
        for (var i = 0; i < game.Players.Length; i++)
        {
            ...
            for (var j = 0; j < dispatcherHand.Count; j++)
            {
                ...
                if (card is PlayerCityCard cityCard && otherPlayer.Location != cityCard.City.Name)
                {
                    yield return new DispatcherDirectFlyPawnCommand(
                        otherPlayer.Role,
                        cityCard.City.Name
                    );
                }
    ...
    {
        if (game.SpecialEventWasRecentlySkipped)
        {
            yield break;
        }
        ...
        foreach (var playerWithCard in game.Players)
        {
            ...
            foreach (var card in cards)
            {
                ...
                {
                    foreach (var x in GovernmentGrants(game, playerWithCard))
                    {
                        yield return x;
                    }
                }
                ...
                {
                    foreach (var x in EventForecasts(game, playerWithCard))
                    {
                        yield return x;
                    }
                }
                ...
                {
                    foreach (var x in Airlifts(game, playerWithCard))
                    {
                        yield return x;
                    }
                }
                ...
                {
                    foreach (var x in ResilientPopulations(game, playerWithCard))
                    {
                        yield return x;
                    }
                }
                ...
                {
                    foreach (var x in OneQuietNights(playerWithCard))
                    {
                        yield return x;
                    }
                }
    ...
    private static IEnumerable<IPlayerCommand> SensibleSpecialEventCommands(PandemicGame game)
    {
        ...
        foreach (var playerWithCard in game.Players)
        {
            ...
            foreach (var card in cards)
            {
                ...
                {
                    foreach (var x in SensibleEventForecasts(game, playerWithCard))
                    {
                        yield return x;
                    }
                }
    ...
    private static IEnumerable<IPlayerCommand> OneQuietNights(Player playerWithCard)
    {
        yield return new OneQuietNightCommand(playerWithCard.Role);
    }

    private static IEnumerable<IPlayerCommand> ResilientPopulations(
        PandemicGame game,
    ...
    private static IEnumerable<IPlayerCommand> EventForecasts(
        PandemicGame game,
        Player playerWithCard
    )
    {
        var perms = new Permutations<InfectionCard>(
            game.InfectionDrawPile.Top(6),
            new InfectionCardComparer()
        );
    ...
    private static IEnumerable<IPlayerCommand> SensibleEventForecasts(
        PandemicGame game,
        Player playerWithCard
    )
    {
        var cards = game
            .InfectionDrawPile.Top(6)
    ...
    private static IEnumerable<IPlayerCommand> GovernmentGrants(
        PandemicGame game,
        Player playerWithCard
    )
    {
        if (game.ResearchStationPile == 0)
        {
            yield break;
        }
    ...
    {
        if (game is { PhaseOfTurn: TurnPhase.DrawCards, CardsDrawn: 1 })
        {
            yield break;
        }

        if (game.PhaseOfTurn is TurnPhase.Epidemic or TurnPhase.EpidemicIntensify)
        {
            yield break;
        }
    ...
    private static IEnumerable<IPlayerCommand> CureCommands(PandemicGame game)
    {
        ...
        )
        {
            if (!game.IsCured(cureCards.Key))
            {
                ...
                yield return new DiscoverCureCommand(
                    game.CurrentPlayer.Role,
                    [.. cureCards.Take(5)]
                );
            }
    ...
    {
        if (
            game.CurrentPlayer.Hand.CityCards().All(c => c.City.Name != game.CurrentPlayer.Location)
        )
        {
            yield break;
        }

        foreach (var city in game.Cities.Select(c => c.Name).Except([game.CurrentPlayer.Location]))
        {
    ...
    private static IEnumerable<IPlayerCommand> ShuttleFlightCommands(PandemicGame game)
    {
        ...
        foreach (
            var city in game
                .Cities.Where(c => c.HasResearchStation)
                .Select(c => c.Name)
    ...
    private static IEnumerable<IPlayerCommand> TreatDiseaseCommands(PandemicGame game)
    {
        ...
        if (cubes.Red > 0)
        {
            yield return new TreatDiseaseCommand(
                game.CurrentPlayer.Role,
                currentLocation,
                Colour.Red
            );
    ...
    private static IEnumerable<IPlayerCommand> ShareKnowledgeGiveCommands(PandemicGame game)
    {
        ...
        )
        {
            foreach (
                var _ in game
                    .CurrentPlayer.Hand.CityCards()
                    .Where(c => c.City.Name == game.CurrentPlayer.Location)
            )
            {
    ...
    private static bool CurrentPlayerCanBuildResearchStation(PandemicGame game)
    {
        ...
        return game
            .CurrentPlayer.Hand.CityCards()
            .Any(c => c.City.Name == game.CurrentPlayer.Location);
    }
}
