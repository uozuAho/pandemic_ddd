...
public static class AllPlayerCommandGenerator
{
    ...
    {
        foreach (var cmd in StandardCommands(game))
        {
            yield return cmd;
        }

        foreach (var cmd2 in SpecialEvents(game))
        {
            yield return cmd2;
        }

        foreach (var cmd3 in RoleSpecialAbilities(game))
        {
            yield return cmd3;
        }
    }
    ...
    private static IEnumerable<IPlayerCommand> SpecialEvents(PandemicGame game)
    {
        foreach (var player in game.Players)
        {
            ...
            yield return new EventForecastCommand(
                player.Role,
                game.InfectionDiscardPile.Top(6).ToImmutableList()
            );
    ...
    {
        foreach (var cmd in DispatcherCommands(game))
        {
            yield return cmd;
        }

        foreach (var cmd in OperationsExpertCommands(game))
        {
            yield return cmd;
        }

        foreach (var cmd in ResearcherCommands(game))
        {
            yield return cmd;
        }

        foreach (var cmd in ScientistCommands(game))
        {
            yield return cmd;
        }

        foreach (var cmd in ContingencyPlannerCommands(game))
        {
            yield return cmd;
        }
    }
    ...
    {
        if (!game.Players.Any(p => p.Role == Role.ContingencyPlanner))
        {
            yield break;
        }
    ...
    {
        if (!game.Players.Any(p => p.Role == Role.Scientist))
        {
            yield break;
        }
    ...
    {
        if (!game.Players.Any(p => p.Role == Role.OperationsExpert))
        {
            yield break;
        }
    ...
    {
        if (!game.Players.Any(p => p.Role == Role.Dispatcher))
        {
            yield break;
        }
    ...
    private static IEnumerable<IPlayerCommand> StandardCommands(PandemicGame game)
    {
        foreach (var city in game.Cities)
        {
            foreach (var player in game.Players)
            {
                yield return new DiscardPlayerCardCommand(
                    player.Role,
                    PlayerCards.CityCard(city.Name)
                );
                ...
                yield return new CharterFlightCommand(
                    player.Role,
                    PlayerCards.CityCard(player.Location),
                    city.Name
                );
        ...
        foreach (var player in game.Players)
        {
            ...
            foreach (
                var cardsToCure in player
                    .Hand.CityCards()
                    .GroupBy(c => c.City.Colour)
                    .Where(g => g.Count() >= 5)
            )
            {
                yield return new DiscoverCureCommand(player.Role, [.. cardsToCure.Select(g => g)]);
            }
